...
export default function DashboardLayout({
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      // Search across multiple entity types using context search
      // Note: userId is now handled server-side via requireAuth
  ...
  const handleResultClick = (result: any) => {
    const { type, id } = result.metadata || {}
    
    // Navigate based on result type
    if (type === 'client_info' || type === 'client') {
      router.push(`/dashboard/contacts?highlight=${id}`)
    } else if (type === 'invoice') {
      router.push(`/dashboard/invoices?highlight=${id}`)
    } else if (type === 'quote') {
      router.push(`/dashboard/quotes?highlight=${id}`)
    }
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-indigo-950 dark:to-purple-950 flex items-center justify-center">
        <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 rounded-2xl p-8 shadow-xl border border-white/20 dark:border-gray-700/50">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto"></div>
          <p className="text-gray-600 dark:text-gray-300 mt-4 text-center">Laden...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-indigo-950 dark:to-purple-950 transition-colors duration-200">
      {/* Top Navigation */}
      <nav className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 shadow-lg border-b border-white/20 dark:border-gray-700/50 transition-colors duration-200 relative z-50">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center flex-shrink-0">
              ...
              <button
                ...
                className="md:hidden p-2 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 mr-3 transition-colors"
            ...
            <div className="hidden md:flex flex-1 max-w-lg mx-8 search-container">
              <div className="relative w-full">
                ...
                <input
                  type="text"
                  placeholder="Zoek facturen, klanten, offertes..."
                  value={searchQuery}
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <span className="text-lg">
                              {result.metadata?.type === 'client_info' || result.metadata?.type === 'client' ? '👤' :
                               result.metadata?.type === 'invoice' ? '📄' :
                               result.metadata?.type === 'quote' ? '📋' : '📎'}
                            </span>
                ...
                {showSearchResults && searchQuery.length >= 2 && !isSearching && searchResults.length === 0 && (
                  <div className="absolute z-50 w-full mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              {/* User Menu */}
              <div className="relative user-menu z-[60]">
                <button
                ...
                {/* Dropdown Menu */}
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-1 z-[100]">
      ...
      {mobileMenuOpen && (
        <>
          ...
          />
          {/* Mobile Sidebar */}
          <div className="fixed inset-y-0 left-0 z-50 w-64 backdrop-blur-xl bg-white/80 dark:bg-gray-800/80 shadow-xl border-r border-white/20 dark:border-gray-700/50 transform transition-transform duration-300 ease-in-out md:hidden">
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between p-4 border-b border-white/20 dark:border-gray-700/50">
                <div className="flex items-center">
                  ...
                  <h2 className="ml-3 text-xl font-bold text-gray-900 dark:text-white">ZzpChat</h2>
                </div>
                <button
                  onClick={() => setMobileMenuOpen(false)}
              ...
              <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
                ...
                <div className="space-y-1">
                  <a
                    ...
                    } group flex items-center px-3 py-2 text-base font-medium rounded-xl transition-colors duration-200`}
                  >
      ...
      <div className="flex mt-4 px-4">
        {/* Sidebar */}
          ...
          <div className="flex flex-col h-[calc(100vh-7rem)] pt-5 pb-2 overflow-y-auto backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 border-r border-white/20 dark:border-gray-700/50 transition-colors duration-200 rounded-t-2xl rounded-b-2xl shadow-xl">
                  ...
                  >
                    ...
                    {!sidebarCollapsed && 'Urenregistratie'}
                      ...
                      <a
                        ...
                        title={sidebarCollapsed ? 'Automatiseringen' : ''}
                      >
                        <svg className={`h-5 w-5 ${
                          pathname === '/dashboard/automations' 
                            ? 'text-indigo-500' 
                        ...
                        {!sidebarCollapsed && 'Automatiseringen'}
                      </a>
                      <a
                        href="/dashboard/documents"
                        className={`${
                        ...
                        title={sidebarCollapsed ? 'Documenten' : ''}
                      >
                        <svg className={`h-5 w-5 ${
                          pathname === '/dashboard/documents' 
                            ? 'text-indigo-500' 
        ...
        {/* Main Content */}
        <div className="flex flex-col flex-1 overflow-x-hidden ml-4 relative z-10">
